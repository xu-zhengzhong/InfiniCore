#include "syr2k_metax.h"
#include "../../../devices/metax/metax_common.h"
#include "../../../devices/metax/metax_handle.h"
#include <algorithm>

namespace op::syr2k::metax {
...
infiniStatus_t Descriptor::create(
    ...
    CHECK_OR_RETURN((info.A_col_stride == 1) == (info.B_col_stride == 1), INFINI_STATUS_BAD_TENSOR_STRIDES);
...
infiniStatus_t Descriptor::calculate(
    ...
    const int B_cols = _info.trans == INFINIOP_BLAS_OP_N ? k : n;

    int lda = utils::cast<int>(_info.A_col_stride);
    int ldb = utils::cast<int>(_info.B_col_stride);
    if (_info.A_col_stride == 1) {
        ...
        ldb = std::max(utils::cast<int>(_info.B_row_stride), B_cols);
    }
