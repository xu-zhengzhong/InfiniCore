...
namespace infinicore::op::trsv_impl::infiniop {

INFINIOP_CACHABLE_DESCRIPTOR(Descriptor, Trsv, 100);
...
void *plan(const Tensor &a, Tensor x, int uplo, int trans, int diag) {
    ...
    INFINIOP_WORKSPACE_TENSOR(workspace, Trsv, descriptor);
...
void run(void *planned_meta) {
    ...
    INFINICORE_CHECK_ERROR(infiniopTrsv(
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(Trsv, &plan, &run, &cleanup);
