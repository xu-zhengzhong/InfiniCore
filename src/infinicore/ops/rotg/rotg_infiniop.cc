...
void calculate(Tensor a, Tensor b, Tensor c, Tensor s) {
    size_t seed = hash_combine(a, b, c, s);
    ...
    if (!desc_opt) {
        INFINICORE_CHECK_ERROR(infiniopCreateRotgDescriptor(
            context::getInfiniopHandle(a->device()), &desc,
            a->desc(), b->desc(), c->desc(), s->desc()));
        cache.put(seed, desc);
    } else {
        desc = *desc_opt;
    }
    ...
    INFINICORE_CHECK_ERROR(infiniopRotg(
        ...
        a->data(), b->data(), c->data(), s->data(), context::getStream()));
}

static bool registered = []() {
    Rotg::dispatcher().registerAll(&calculate, false);
    return true;
}();
