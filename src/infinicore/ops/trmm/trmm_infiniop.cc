...
namespace infinicore::op::trmm_impl::infiniop {

INFINIOP_CACHABLE_DESCRIPTOR(Descriptor, Trmm, 100);
...
void *plan(const Tensor &a, const Tensor &alpha, Tensor b, int side, int uplo, int trans, int diag) {
    ...
    INFINIOP_WORKSPACE_TENSOR(workspace, Trmm, descriptor);
...
void run(void *planned_meta) {
    ...
    INFINICORE_CHECK_ERROR(infiniopTrmm(
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(Trmm, &plan, &run, &cleanup);
