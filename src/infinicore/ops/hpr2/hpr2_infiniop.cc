...
namespace infinicore::op::hpr2_impl::infiniop {

INFINIOP_CACHABLE_DESCRIPTOR(Descriptor, Hpr2, 100);
...
void *plan(const Tensor &alpha, const Tensor &x, const Tensor &y, Tensor ap, int uplo) {
    ...
    INFINIOP_CACHABLE_DESCRIPTOR_GET_OR_CREATE(
        Descriptor, descriptor, Hpr2,
        seed,
        static_cast<infiniopBlasFillMode_t>(uplo),
        alpha->desc(), x->desc(), y->desc(), ap->desc());

    INFINIOP_WORKSPACE_TENSOR(workspace, Hpr2, descriptor);
...
void run(void *planned_meta) {
    ...
    INFINICORE_CHECK_ERROR(infiniopHpr2(
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(Hpr2, &plan, &run, &cleanup);
