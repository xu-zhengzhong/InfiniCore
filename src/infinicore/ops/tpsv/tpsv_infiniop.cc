...
namespace infinicore::op::tpsv_impl::infiniop {

INFINIOP_CACHABLE_DESCRIPTOR(Descriptor, Tpsv, 100);
...
void *plan(const Tensor &ap, Tensor x, int uplo, int trans, int diag) {
    ...
    INFINIOP_WORKSPACE_TENSOR(workspace, Tpsv, descriptor);
...
void run(void *planned_meta) {
    ...
    INFINICORE_CHECK_ERROR(infiniopTpsv(
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(Tpsv, &plan, &run, &cleanup);
