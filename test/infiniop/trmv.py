...
def test(
    ...
    print(
        f"Testing Trmv on {InfiniDeviceNames[device]} with uplo:{uplo} trans:{trans} diag:{diag} n:{n} "
    ...
    check_error(
        LIBINFINIOP.infiniopCreateTrmvDescriptor(
            handle,
    ...
    check_error(
        LIBINFINIOP.infiniopGetTrmvWorkspaceSize(
    ...
    if PROFILE:
        ...
        profile_operation(
            "    lib", lambda: lib_trmv(), device, NUM_PRERUN, NUM_ITERATIONS
        )

    check_error(LIBINFINIOP.infiniopDestroyTrmvDescriptor(descriptor))
