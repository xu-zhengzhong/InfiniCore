...
def test(
    ...
    # nrm2 returns a scalar float. We use c_float to capture the result.
    ...
    expected_val = torch_nrm2(x.torch_tensor()).item()
    ...
    if PROFILE:
        # fmt: off
        profile_operation("PyTorch", lambda: torch_nrm2(x.torch_tensor()), device, NUM_PRERUN, NUM_ITERATIONS)
