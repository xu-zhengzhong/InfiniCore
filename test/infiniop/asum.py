...
def asum(x, out):
    out.copy_(torch.sum(torch.abs(x)))
...
def test(
    ...
    asum(x.torch_tensor(), result.torch_tensor())
    ...
    if PROFILE:
        # fmt: off
        profile_operation("PyTorch", lambda: torch.sum(torch.abs(x.torch_tensor().reshape(-1))), device, NUM_PRERUN, NUM_ITERATIONS)
