...
_STRIDED_SHAPES = [
    ...
    (32, 31, (40, 1), (48, 1), (37, 1)),
    (128, 3, (1, 136), (1, 8), (1, 136)),
    (3, 128, (1, 8), (1, 136), (1, 8)),
]
...
_TEST_CASES = [
    ...
    for n, k, a_stride_n, a_stride_t, c_stride in _STRIDED_SHAPES
    for uplo, trans in _UPLO_TRANS_CASES
]
...
def test(
    ...
    print(
        f"Testing Syrk on {InfiniDeviceNames[device]} with uplo:{uplo} trans:{trans} n:{n} k:{k} "
    ...
    product = (
        torch.mm(A.torch_tensor(), A.torch_tensor().t())
        if trans == 0
        else torch.mm(A.torch_tensor().t(), A.torch_tensor())
    )
    ...
    check_error(
        LIBINFINIOP.infiniopCreateSyrkDescriptor(
            handle,
    ...
    check_error(
        LIBINFINIOP.infiniopGetSyrkWorkspaceSize(
    ...
    if PROFILE:
        ...
        profile_operation(
            "    lib", lambda: lib_syrk(), device, NUM_PRERUN, NUM_ITERATIONS
        )

    check_error(LIBINFINIOP.infiniopDestroySyrkDescriptor(descriptor))
