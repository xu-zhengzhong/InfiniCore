...
def test(handle, device, d1_0, d2_0, x1_0, y1_0, dtype=torch.float32, sync=None):
    ...
    d1 = TestTensor((1,), (1,), dtype, device, mode="manual", set_tensor=torch.tensor([d1_0], dtype=scalar_dtype))
    d2 = TestTensor((1,), (1,), dtype, device, mode="manual", set_tensor=torch.tensor([d2_0], dtype=scalar_dtype))
    x1 = TestTensor((1,), (1,), dtype, device, mode="manual", set_tensor=torch.tensor([x1_0], dtype=scalar_dtype))
    y1 = TestTensor((1,), (1,), dtype, device, mode="manual", set_tensor=torch.tensor([y1_0], dtype=scalar_dtype))
    ...
    for i in range(5):
        assert math.isclose(param.actual_tensor()[i].item(), exp_sparam[i], rel_tol=rtol, abs_tol=atol)
