...
def test(
    ...
    print(
        f"Testing Spr2 on {InfiniDeviceNames[device]} with uplo:{uplo} n:{n} "
    ...
    AP_ref = _packed_rank2_update(
        AP.torch_tensor(), x.torch_tensor(), y.torch_tensor(), alpha.torch_tensor(), uplo, n
    )
    ...
    check_error(
        LIBINFINIOP.infiniopCreateSpr2Descriptor(
            handle,
    ...
    check_error(
        LIBINFINIOP.infiniopGetSpr2WorkspaceSize(
    ...
    if PROFILE:
        ...
        profile_operation(
            "    lib", lambda: lib_spr2(), device, NUM_PRERUN, NUM_ITERATIONS
        )

    check_error(LIBINFINIOP.infiniopDestroySpr2Descriptor(descriptor))
