...
def test(
    ...
    print(
        f"Testing Her on {InfiniDeviceNames[device]} with uplo:{uplo} n:{n} "
    ...
    A_ref = her(alpha.torch_tensor(), A.torch_tensor(), x.torch_tensor(), uplo)
    ...
    check_error(
        LIBINFINIOP.infiniopCreateHerDescriptor(
            handle,
    ...
    for tensor in [alpha, x, A]:
        tensor.destroy_desc()

    workspace_size = c_uint64(0)
    check_error(
        LIBINFINIOP.infiniopGetHerWorkspaceSize(
    ...
    if PROFILE:
        profile_operation(
            "PyTorch",
            lambda: her(alpha.torch_tensor(), A.torch_tensor(), x.torch_tensor(), uplo),
            device,
            NUM_PRERUN,
            NUM_ITERATIONS,
        )
        profile_operation(
            "    lib", lambda: lib_her(), device, NUM_PRERUN, NUM_ITERATIONS
        )

    check_error(LIBINFINIOP.infiniopDestroyHerDescriptor(descriptor))
