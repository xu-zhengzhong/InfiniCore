...
def test(
    ...
    print(
        f"Testing Syr on {InfiniDeviceNames[device]} with uplo:{uplo} n:{n} "
    ...
    update = alpha.torch_tensor() * torch.outer(x.torch_tensor(), x.torch_tensor())
    ...
    check_error(
        LIBINFINIOP.infiniopCreateSyrDescriptor(
            handle,
    ...
    check_error(
        LIBINFINIOP.infiniopGetSyrWorkspaceSize(
    ...
    if PROFILE:
        profile_operation(
            ...
            lambda: _triangle_update(
                A.torch_tensor(),
                alpha.torch_tensor() * torch.outer(x.torch_tensor(), x.torch_tensor()),
                uplo,
            ),
            device,
            NUM_PRERUN,
        ...
        profile_operation(
            "    lib", lambda: lib_syr(), device, NUM_PRERUN, NUM_ITERATIONS
        )

    check_error(LIBINFINIOP.infiniopDestroySyrDescriptor(descriptor))
